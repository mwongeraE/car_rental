...
public class CalendarDialogBuilder {
  ...
  public AlertDialog.Builder returnDialog() {
    ...
    LinearLayout ll = (LinearLayout) inflater.inflate(R.layout.layout_calendar_dialog, null, false);
  ...
  private void configCalendarView() {
    ...
    mCv.setBackgroundColor(context.getResources().getColor(R.color.Orange));

    // theses methods are API 16+ only.
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
      //The background color for the selected week.
      mCv.setSelectedWeekBackgroundColor(context.getResources().getColor(R.color.OrangeRed));

      //sets the color for the vertical bar shown at the beginning and at the end of the selected date.
      mCv.setSelectedDateVerticalBar(new ColorDrawable(context.getResources().getColor(R.color.Green)));

      //sets the color for the dates of an unfocused month.
      mCv.setUnfocusedMonthDateColor(context.getResources().getColor(R.color.Gray));
